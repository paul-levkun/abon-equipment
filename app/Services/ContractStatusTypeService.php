...
class ContractStatusTypeService {
  ...
  protected function index(Request $req) { 
    try {
      ...
      return json_encode([ 
        "count" => ContractStatusType::where("is_deleted", false)->count(),
        "dict" => ContractStatusType::where("is_deleted", false)->offset($offset)->limit($page_size)->get()
      ]);
    }
    catch (\Exception $e) {
  ...
  protected function show(Request $req) { 
    return json_encode( 
      ContractStatusType::where("id", $req['row_id'])->get()
    );
  }
  ...
  protected function update(Request $req) { 
    ...
    $ds_old = ContractStatusType::where("id", $req->row_id)->get();

    $model = ContractStatusType::find($req->row_id);
    $model->name = $req->name;
  ...
  protected function destroy(Request $req) { 
    ...
    // ContractStatusType::where("id", $req["row_id"])->delete();
  ...
  /////////////////////////////////////// Задіяти Requests
	...
	private function fieldValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'name' => 'required|unique:contract_status_types,name,' . $req->row_id . ',id,is_deleted,0',
		]);
		if ($validator->fails()) {
