...
class SapRemService {
  ...
  protected function index(Request $req) { 
    try {
      ...
      // return json_encode([ 
      //   "count" => SapRem::where("active", true)->count(),
      //   "dict" => SapRem::where("active", true)->offset($offset)->limit($page_size)->get()
      // ]);
      return json_encode([ 
        "count" => SapRem::with('sap_sos')->where("active", true)->count(),
        "dict" => SapRem::with('sap_sos')->where("active", true)->offset($offset)->limit($page_size)->get()
      ]);
    }
    catch (\Exception $e) {
  ...
  protected function show(Request $req) { 
    // return json_encode( 
    //   SapRem::where("id", $req['row_id'])->get()
    // );
  }
  ...
  protected function destroy(Request $req) { 
    ...
    // SapRem::where("id", $req["row_id"])->delete();
  }
  ...
  /////////////////////////////////////// Задіяти Requests
	...
	// 		'name' => 'required|unique:sap_rems,name',
	// 	]);
	// 	if ($validator->fails()) {
