...
class DocTypeService {
  ...
  protected function index(Request $req) { 
    try {
      ...
      return json_encode([ 
        "count" => DocType::where("is_deleted", false)->count(),
        "dict" => DocType::where("is_deleted", false)->offset($offset)->limit($page_size)->get()
      ]);
    }
    catch (\Exception $e) {
  ...
  protected function show(Request $req) { 
    return json_encode( 
      DocType::where("id", $req['row_id'])->get()
    );
  }
  ...
  protected function update(Request $req) { 
    ...
    $ds_old = DocType::where("id", $req->row_id)->get();

    $model = DocType::find($req->row_id);
    $model->name = $req->name;
  ...
  protected function destroy(Request $req) { 
    ...
    // DocType::where("id", $req["row_id"])->delete();
  ...
  /////////////////////////////////////// Задіяти Requests
	...
	private function fieldValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'name' => 'required|unique:doc_types,name,' . $req->row_id . ',id,is_deleted,0',
		]);
		if ($validator->fails()) {
	...
	private function destroyValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'row_id' => 'unique:documents,doc_type_id'
		]);
		if ($validator->fails()) {
