...
class EquipmentTypeService {
  ...
  protected function index(Request $req) { 
    try {
      ...
      return json_encode([ 
        ...
        "dict" => EquipmentType::offset($offset)->limit($page_size)->get()
      ]);
    }
    catch (\Exception $e) {
  ...
  protected function show(Request $req) { 
    return json_encode( 
      EquipmentType::where("id", $req['row_id'])->get()
    );
  }
  ...
  protected function update(Request $req) { 
    ...
    $model = EquipmentType::find($req->row_id);
    $model->name = $req->name;
    $model->save();
  }
  ...
  protected function destroy(Request $req) { 
    ...
    EquipmentType::where("id", $req["row_id"])->delete();
  }
  ...
  /////////////////////////////////////// Задіяти Requests
	...
	private function fieldValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'name' => 'required|unique:equipment_types,name,' . $req->row_id,
		]);
		if ($validator->fails()) {
