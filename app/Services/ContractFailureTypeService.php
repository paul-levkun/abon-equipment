...
class ContractFailureTypeService {
  ...
  protected function index(Request $req) { 
    try {
      ...
      return json_encode([ 
        "count" => ContractFailureType::where("is_deleted", false)->count(),
        "dict" => ContractFailureType::where("is_deleted", false)->offset($offset)->limit($page_size)->get()
      ]);
    }
    catch (\Exception $e) {
  ...
  protected function show(Request $req) { 
    return json_encode( 
      ContractFailureType::where("id", $req['row_id'])->get()
    );
  }
  ...
  protected function update(Request $req) { 
    ...
    $ds_old = ContractFailureType::where("id", $req->row_id)->get();

    $model = ContractFailureType::find($req->row_id);
    $model->name = $req->name;
    $model->user_id = $req->user_id;
    $model->save();

    LogAction::create([
      'table_id' => "contract_failure_types",
      'row_id' => $req->row_id,
  ...
  protected function destroy(Request $req) { 
    ...
    // ContractFailureType::where("id", $req["row_id"])->delete();
  ...
  /////////////////////////////////////// Задіяти Requests
	...
	private function fieldValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'name' => 'required|unique:contract_failure_types,name,' . $req->row_id . ',id,is_deleted,0',
		]);
		if ($validator->fails()) {
	...
	private function destroyValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'row_id' => 'unique:registers,contract_failure_type_id'
		]);
		if ($validator->fails()) {
