...
class StatusTypeService {
  ...
  protected function index(Request $req) { 
    try {
      ...
      return json_encode([ 
        "count" => StatusType::count(),
        "dict" => StatusType::offset($offset)->limit($page_size)->get()
      ]);
    }
    catch (\Exception $e) {
  ...
  protected function show(Request $req) { 
    return json_encode( 
      StatusType::where("id", $req['row_id'])->get()
    );
  }
  ...
  protected function update(Request $req) { 
    ...
    $model = StatusType::find($req->row_id);
    $model->name = $req->name;
    $model->save();
  }
  ...
  protected function destroy(Request $req) { 
    ...
    StatusType::where("id", $req["row_id"])->delete();
  }
  ...
  /////////////////////////////////////// Задіяти Requests
	...
	private function fieldValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'name' => 'required|unique:status_types,name,' . $req->row_id . ',id,is_deleted,0',
		]);
		if ($validator->fails()) {
