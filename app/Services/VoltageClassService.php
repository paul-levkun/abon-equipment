...
use App\Models\VoltageClass;

class VoltageClassService {
  ...
  protected function index(Request $req) { 
    try {
      ...
      return json_encode([ 
        "count" => VoltageClass::count(),
        "dict" => VoltageClass::orderBy('sort')->offset($offset)->limit($page_size)->get()
      ]);
    }
    catch (\Exception $e) {
  ...
  protected function show(Request $req) { 
    // return json_encode( 
    //   VoltageClass::where("id", $req['row_id'])->get()
    // );
  }
  ...
  protected function update(Request $req) { 
    ...
    // $model = VoltageClass::find($req->row_id);
    // $model->name = $req->name;
    // $model->save();
  }
  ...
  protected function destroy(Request $req) { 
    ...
    // VoltageClass::where("id", $req["row_id"])->delete();
  }
