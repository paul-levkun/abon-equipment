...
class ContractInexpediencyTypeService {
  ...
  protected function index(Request $req) { 
    try {
      ...
      return json_encode([ 
        "count" => ContractInexpediencyType::where("is_deleted", false)->count(),
        "dict" => ContractInexpediencyType::where("is_deleted", false)->offset($offset)->limit($page_size)->get()
      ]);
    }
    catch (\Exception $e) {
  ...
  protected function show(Request $req) { 
    return json_encode( 
      ContractInexpediencyType::where("id", $req['row_id'])->get()
    );
  }
  ...
  try {
    ...
    ContractInexpediencyType::create([
      'name' => $req->name,
      'user_id' => $req->user_id,
    ]);  
  }
  ...
  protected function update(Request $req) { 
    ...
    $ds_old = ContractInexpediencyType::where("id", $req->row_id)->get();

    $model = ContractInexpediencyType::find($req->row_id);
    $model->name = $req->name;
  ...
  protected function destroy(Request $req) { 
    ...
    // ContractInexpediencyType::where("id", $req["row_id"])->delete();
  ...
  /////////////////////////////////////// Задіяти Requests
	...
	private function fieldValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'name' => 'required|unique:contract_inexpediency_types,name,' . $req->row_id . ',id,is_deleted,0',
		]);
		if ($validator->fails()) {
	...
	private function destroyValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'row_id' => 'unique:registers,contract_inexpediency_type_id'
		]);
		if ($validator->fails()) {
