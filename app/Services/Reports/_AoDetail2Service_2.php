...
class AoDetail2Service {
  ...
  public function makeReport() {
		...
		$activeWorksheet->mergeCells('J4:L4');
		$activeWorksheet->mergeCells('M4:O4');
		$activeWorksheet->mergeCells('P4:S4');
		...
		$activeWorksheet->setCellValue("F5", "SAP код");
		...
		$activeWorksheet->setCellValue("J5", "SAP код");
		$activeWorksheet->setCellValue("K5", "Назва");
		$activeWorksheet->setCellValue("L5", "Тип");

		$activeWorksheet->setCellValue("M4", "Договір");
		$activeWorksheet->setCellValue("M5", "№ SAP договору");
		$activeWorksheet->setCellValue("N5", "Дата початку");
		$activeWorksheet->setCellValue("O5", "Дата завершення");

		$activeWorksheet->setCellValue("P4", "Інформація по договору");
		$activeWorksheet->setCellValue("P5", "Статус");
		$activeWorksheet->setCellValue("Q5", "Тип");
		$activeWorksheet->setCellValue("R5", "Відмова");
    	...
    	->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
		$activeWorksheet->getStyle("M{$iStartRowIdx}:{$sFinishCol}{$iFinishRow}")
    	->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
		...
		$activeWorksheet->getColumnDimension('M')->setWidth(12);
		$activeWorksheet->getColumnDimension('N')->setWidth(14);
    ...
    // 	->getBorders()->getAllBorders()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
		...
		foreach($dsRegister as $register) {
			...
			$activeWorksheet->setCellValue("M{$iCurrRow}", $register->sap_code_contract);
			$activeWorksheet->setCellValue("N{$iCurrRow}", $register->start_date_contract);
			$activeWorksheet->setCellValue("O{$iCurrRow}", $register->end_date_contract);
			$activeWorksheet->setCellValue("P{$iCurrRow}", $register->contract_status_type);
			$activeWorksheet->setCellValue("Q{$iCurrRow}", $register->contract_type);
			$activeWorksheet->setCellValue("R{$iCurrRow}", $register->contract_failure_type);
