...
class LineTypeService {
  ...
  protected function index(Request $req) { 
    try {
      ...
      return json_encode([ 
        ...
        "dict" => LineType::offset($offset)->limit($page_size)->get()
      ]);
    }
    catch (\Exception $e) {
  ...
  protected function show(Request $req) { 
    return json_encode( 
      LineType::where("id", $req['row_id'])->get()
    );
  }
  ...
  protected function update(Request $req) { 
    ...
    $model = LineType::find($req->row_id);
    $model->name = $req->name;
    $model->save();
  }
  ...
  protected function destroy(Request $req) { 
    ...
    LineType::where("id", $req["row_id"])->delete();
  }
  ...
  /////////////////////////////////////// Задіяти Requests
	...
	private function fieldValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'name' => 'required|unique:line_types,name,' . $req->row_id . ',id,is_deleted,0',
		]);
		if ($validator->fails()) {
