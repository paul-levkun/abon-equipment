...
class ObjectTypeService {
  ...
  protected function index(Request $req) { 
    try {
      ...
      return json_encode([ 
        "count" => ObjectType::where("active", true)->count(),
        "dict" => ObjectType::where("active", true)->offset($offset)->limit($page_size)->get()
      ]);
    }
    catch (\Exception $e) {
  ...
  protected function show(Request $req) { 
    return json_encode( 
      ObjectType::where("id", $req['row_id'])->get()
    );
  }
  ...
  protected function update(Request $req) { 
    ...
    $model = ObjectType::find($req->row_id);
    $model->name = $req->name;
    $model->save();
  }
  ...
  protected function destroy(Request $req) { 
    ...
    ObjectType::where("id", $req["row_id"])->delete();
  }
  ...
  /////////////////////////////////////// Задіяти Requests
	...
	private function fieldValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'name' => 'required|unique:object_types,name,' . $req->row_id . ',id,is_deleted,0',
		]);
		if ($validator->fails()) {
