...
class SubstationService {
  ...
  protected function index(Request $req) { 
    try {
      ...
      if (isset($req["object_type_id"]) && $req["object_type_id"] !== "0") {
        ...
        // Log::error($req["object_type_id"]);
        $arr = explode(",", $req["object_type_id"]);
        $query = $query->whereIn("object_type_id", $arr);
      }
  ...
  protected function show(Request $req) { 
    return json_encode( 
      Substation::where("id", $req['row_id'])->get()
    );
  }
  ...
  protected function store(Request $req) { 
    ...
    // Substation::create([
    //   "id_res" => $req["rem"], "id_type_Substation" => $req["type"], "ab_inn" => $req["code"], 
  ...
  protected function update(Request $req) { 
    ...
    // $model = Substation::find($req->row_id);
    // $model["id_res"] = $req["rem"]; $model["id_type_Substation"] = $req["type"]; $model["ab_inn"] = $req["code"]; 
  ...
  protected function destroy(Request $req) { 
    ...
    // Substation::where("id", $req["row_id"])->delete();
  }
  ...
  // try{
	...
	// 		'code' => 'required|unique:substations,ab_inn,' . $req->row_id,
	// 	]);
	// 	if ($validator->fails()) {
	...
	// 	$validator = Validator::make($req->all(), [
	// 		'name' => 'required|unique:Substations,name,' . $req->row_id,
	// 	]);
	// 	if ($validator->fails()) {
  ...
  // }
	...
	// 	// $validator = Validator::make($req->all(), [
	// 	// 	'row_id' => 'unique:substations,id_type_substation'
	// 	// ]);
	// 	// if ($validator->fails()) {
