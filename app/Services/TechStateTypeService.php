...
class TechStateTypeService {
  ...
  protected function index(Request $req) { 
    try {
      ...
      return json_encode([ 
        "count" => TechStateType::where("is_deleted", false)->count(),
        "dict" => TechStateType::where("is_deleted", false)->offset($offset)->limit($page_size)->get()
      ]);
    }
    catch (\Exception $e) {
  ...
  protected function show(Request $req) { 
    return json_encode( 
      TechStateType::where("id", $req['row_id'])->get()
    );
  }
  ...
  protected function update(Request $req) { 
    ...
    $ds_old = TechStateType::where("id", $req->row_id)->get();

    $model = TechStateType::find($req->row_id);
    $model->name = $req->name;
    $model->user_id = $req->user_id;
    $model->save();

    LogAction::create([
      'table_id' => "tech_state_types",
      'row_id' => $req->row_id,
  ...
  protected function destroy(Request $req) { 
    ...
    // TechStateType::where("id", $req["row_id"])->delete();
  ...
  /////////////////////////////////////// Задіяти Requests
	...
	private function fieldValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'name' => 'required|unique:tech_state_types,name,' . $req->row_id . ',id,is_deleted,0',
		]);
		if ($validator->fails()) {
	...
	private function destroyValidate($req) {
		...
		$validator = Validator::make($req->all(), [
			'row_id' => 'unique:registers,tech_state_type_id'
		]);
		if ($validator->fails()) {
