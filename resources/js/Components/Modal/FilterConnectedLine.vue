...
export default {
  ...
  methods: {
    submitForm(sap_so_id, sap_rem_id) {
      ...
      this.$store.commit("dictModule/setFilterConnectedLineParams", filterParams);
  ...
  created() {
    console.log("FilterConnectedLine created");
    this.$store.dispatch("dictModule/setRef", { id: "status_types" });
    this.$store.dispatch("dictModule/setRef", { id: "object_types" });
  },
  mounted() {
    const filterParams = this.$store.getters["dictModule/filterConnectedLineParams"];
