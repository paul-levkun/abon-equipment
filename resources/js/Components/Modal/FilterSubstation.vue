...
export default {
  ...
  created() {
    console.log("FilterSubstation created");
    this.$store.dispatch("dictModule/setRef", { id: "status_types" });
    this.$store.dispatch("dictModule/setRef", { id: "object_types" });
  },
  mounted() {
    console.log(this.$store.getters["dictModule/filterSubstationParams"]);
    const filterParams = this.$store.getters["dictModule/filterSubstationParams"];
