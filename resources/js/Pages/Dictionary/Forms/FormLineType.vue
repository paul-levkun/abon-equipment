...
export default {
  ...
  methods: {
    submitForm() {
      ...
      data.append('id', "line_types")
      data.append('name', this.name)
      data.append('row_id', this.row_id)
      if (this.row_id)
        data.append("_method", "put");

      this.$store.dispatch('dictModule/storeDict', { "id": "line_types", "oper": this.row_id ? "upd" : "add", "data": data } )
    },
    clearValidity(inputId) {
      // console.log("cvcvcvcvcvc")
  ...
  created() {
    console.log("FormLineType created")
  },
  mounted() {
