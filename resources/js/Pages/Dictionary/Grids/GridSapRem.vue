  ...
  <DictGridContent :clickedRow="clickedRow">
    <template #thead>
      <tr>
        ...
        <th class="sticky top-0 p-2 border-collapse border border-slate-400 bg-slate-300">СО</th>
        <th class="sticky top-0 p-2 border-collapse border border-slate-400 bg-slate-300">Код РЕМ</th>
    ...
    <template #tbody="{ currentDict, pageId }">
      <tr v-for="(row, index) in currentDict" :key="row.id" class="bg-white tr" 
        ...
        <td class="p-1 border-collapse border border-slate-400">{{ row.sap_sos.name }}</td>
        <td class="p-1 border-collapse border border-slate-400">{{ row.code_rem }}</td>
