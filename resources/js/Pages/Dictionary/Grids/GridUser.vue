<script setup>
  import DictGridContent from './../../../Layouts/DictGridContent.vue';
</script>

<template>
  <DictGridContent :clickedRow="clickedRow">
    <template #thead>
      <tr>
        <th class="sticky top-0 p-2 border-collapse border border-slate-400 bg-slate-300 w-20">№ з/п</th>
        <th class="sticky top-0 p-2 border-collapse border border-slate-400 bg-slate-300">Назва</th>
        <th class="sticky top-0 p-2 border-collapse border border-slate-400 bg-slate-300">Email</th>
        <th class="sticky top-0 p-2 border-collapse border border-slate-400 bg-slate-300">Повноваження</th>
      </tr>
    </template>

    <template #tbody="{ currentDict, pageId }">
      <tr v-for="(row, index) in currentDict" :key="row.id" class="bg-white tr" 
          @click="rowClick(row.id, row.isSelected)" 
          :class="{ 'tr-selected': row.isSelected }">
        <td class="p-1 border-collapse border border-slate-400 text-center">{{ (pageId - 1) * 20 + index + 1 }}</td>
        <td class="p-1 border-collapse border border-slate-400">{{ row.name }}</td>
        <td class="p-1 border-collapse border border-slate-400">{{ row.email }}</td>
        <td class="p-1 border-collapse border border-slate-400">

          <b><p class="text-red-700">{{ row.so_id === -1 ? "СО: Права відсутні" : "" }}</p></b>
          <b><p>{{ row.so_id === 0 ? "СО: Всі" : "" }}</p></b>
          <b><p>{{ row.so_id > 0 ? `СО: ${row.sap_sos.code_so} ${row.sap_sos.name}` : "" }}</p></b>

          <p>{{ row.sap_rems ? `&nbsp;&nbsp;${row.sap_rems.code_rem} ${row.sap_rems.name}` : "" }}</p>

          <i><b><p class="text-red-700">{{ row.bo_id === -1 ? "БО: Права відсутні" : "" }}</p></b></i>
          <i><b><p class="text-green-700">{{ row.bo_id === 0 ? "БО: Всі" : "" }}</p></b></i>
          <i><b><p class="text-green-700">{{ row.bo_id > 0 ? `БО: ${row.sap_bos.code_bo} ${row.sap_bos.name}` : "" }}</p></b></i>
        </td>
      </tr>
    </template>
  </DictGridContent>
</template>

<script>
export default {
  emits: ["showActiveRow"],
  data() {
    return {
      clickedRow: { id: null, isSelected: false },
    }
  },
  methods: {
    rowClick(id, isSelected) {
      this.clickedRow.id = id
      this.clickedRow.isSelected = !isSelected
      this.$emit('showActiveRow', this.clickedRow)
    },

  },
  computed: {
  }
}
</script>
