  ...
  <DictGridContent :clickedRow="clickedRow">
    <template #thead>
      <tr>
        ...
        <th class="sticky top-0 p-2 border-collapse border border-slate-400 bg-slate-300">Код ЕМ</th>
    ...
    <template #tbody="{ currentDict, pageId }">
      <tr v-for="(row, index) in currentDict" :key="row.id" class="bg-white tr" 
        ...
        <td class="p-1 border-collapse border border-slate-400">{{ row.sap_so }}</td>
        <td class="p-1 border-collapse border border-slate-400">{{ row.sap_loc }}</td>
