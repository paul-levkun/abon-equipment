  ...
  <DictGridContent :clickedRow="clickedRow" @delete-click-reset="deleteClickReset">
    ...
    <template #tbody="{ currentDict, pageId }">
      ...
      >
        ...
        <td class="p-1 border-collapse border border-slate-400">{{ row.sap_code }}</td>
        <td class="p-1 border-collapse border border-slate-400">{{ row.ab_inn }}</td>
        <td class="p-1 border-collapse border border-slate-400">{{ row.name }}</td>
        <td class="p-1 border-collapse border border-slate-400">{{ row.recvisit }}</td>
        <td class="p-1 border-collapse border border-slate-400">{{ row.comments }}</td>
      </tr>
    </template>
  </DictGridContent>
</template>
